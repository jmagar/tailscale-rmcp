...
function log(message) {
  process.stderr.write(`tailscale-rmcp: ${message}\n`);
}
...
async function main() {
  if (process.env.TAILSCALE_RMCP_SKIP_DOWNLOAD === "1") {
    log("skipping binary download because TAILSCALE_RMCP_SKIP_DOWNLOAD=1");
    return;
  }
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "tailscale-rmcp-install-"));
